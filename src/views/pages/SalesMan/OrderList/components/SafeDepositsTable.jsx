...
import { DeleteSalesmanById, GetSalesmenPaging } from '@/services/ModelSserver/SalesmanServices';
...
const ActionColumn = ({
    ...
    const onView = () => {
        handleSelectItem();
        navigate(`/Banks/view/${row.id}`);
    };

    const onEdit = () => {
        // Fixed navigation path to match protectedRoutes
        navigate(`/SalesMan/Edit/${row.id}`);
    };
...
const SafeDepositsTable = ({
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const result = await GetSalesmenPaging({
    ...
    const columns = useMemo(
        () => [
            ...
            {
                header: t(`${translationPath}firmNo`),
                accessorKey: 'firmNo',
                sortable: true,
                cell: (props) => <span>{props.row.original.firmNo}</span>,
            },
            {
                header: t(`${translationPath}position`),
                accessorKey: 'position',
                sortable: true,
                cell: (props) => <span>{props.row.original.position}</span>,
            },
            {
    ...
    const dialogType = {
        type: 'danger',
        title: t(`${translationPath}deleteSalesMan`),
    ...
    const handleonConfirm = async () => {
        ...
        try {
            const response = await DeleteSalesmanById(selectItem.id);
